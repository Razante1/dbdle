...
function App() {
  ...
  const enviarEscolha = (name) => {
    ...
    if (resultado) {
      ...
      setPersonagemComparado(comparado);  // salva versão correta para comparação
  ...
  // Calcula o número de colunas para o grid (exclui 'image')
  const colunas = Object.keys(personagemDaVez).filter(k => k !== "image").length;
  ...
  return (
    <div style={{
      ...
      >
        {!venceu &&
          Object.entries(personagemDaVez)
            .filter(([k]) => k !== "image")
            .map(([chave], index) => (
              <div
                className="atributoTitulo"
                key={index}
                style={{
                  fontWeight: "bolder",
                  color: "white",
